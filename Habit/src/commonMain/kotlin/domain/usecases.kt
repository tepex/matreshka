...
/**
 * Расчитывает состояние матрицы тепловой карты (Heatmap) для конкретной привычки.
 *
 * Генерация сетки дней строится в обратном хронологическом порядке построчно, где самый нижний ряд всегда представляет
 * текущую календарную неделю. Таблица строго начинается с Понедельника и заканчивается Воскресеньем.
 *
 * Каждая ячейка может принимать три состояния:
 * - [HeatmapState.COMPLETED]: Привычка запланирована и успешно выполнена.
 * - [HeatmapState.NOT_COMPLETED]: Привычка запланирована, но пропущена (только для прошлых дней и сегодня).
 * - [HeatmapState.EMPTY]: Будущие дни текущей недели, либо дни недели, не входящие в расписание.
 *
 * @param habit Целевая привычка, для которой строится сетка.
 * @param habitFactRepository Репозиторий для получения дат выполнения фактов.
 * @param today Текущая дата на устройстве, служащая опорной точкой «сегодня».
 * @param period Выбранный временной диапазон анализа (1, 2 или 4 недели).
 * @return [HeatmapUiState], содержащий список строк [HeatmapRow] фиксированной ширины в 7 дней.
 *
 * @see docs/statistics.adoc
 */
